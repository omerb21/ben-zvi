...
def _canonicalize_personal_number(value: str) -> str:
    raw_value = _strip_or_empty(value)
    if not raw_value:
        return ""

    canonical = raw_value
    start = raw_value.find("(")
    end = raw_value.find(")", start + 1) if start != -1 else -1
    if start != -1 and end != -1 and end > start + 1:
        inner = raw_value[start + 1 : end].strip()
        if inner:
            canonical = inner
...
def _snapshot_hash_part(
    ...
    return hashlib.md5(
        f"{fund_code}|{fund_number}|{fund_name}|{fund_type}".encode("utf-8")
    ).hexdigest()[:8]
...
def _select_latest_snapshots_by_key(snapshots: List[Snapshot]) -> Dict[tuple[str, str], Snapshot]:
    ...
    for s in snapshots:
        fund_code = _strip_or_empty(s.fund_code)
        fund_number = _strip_or_empty(s.fund_number)
...
def _match_saving_product_for_snapshot(
    ...
    *,
    ...
    if snap.fund_name and snap.source:
        ...
        fund_name_raw = _strip_or_empty(snap.fund_name)

        expected_norm = _normalize_company_for_match(expected_company)
        for candidate in saving_products:
            if _strip_or_empty(candidate.fund_name) != fund_name_raw:
                continue
            cand_norm = _normalize_company_for_match(candidate.company_name or "")
    ...
    if sp is None:
        clean_code = _strip_or_empty(snap.fund_code)
        if clean_code:
